...
<div class="layui-layout layui-layout-admin">
	...
	<div class="layui-body" style="background-color: #e6e6e6">
		...
		<div style="margin:0;background-color: white">
			<form class="layui-form" action="${ctx}/backendApp/select" method="post" style="padding-top: 20px">
				<%--分页查询条件--%>
				...
				<tbody>
					<c:forEach items="${pageInfo.list}" var="obj">
						<tr>
							...
							<td>
 								<%--对待审核状态的app进行审核操作--%>
								<c:if test="${obj.appStatus.valueId == 1 || obj.appStatus.valueName == '待审核'}">
									<a href="${ctx}/backendApp/toCheck/${obj.id}">
										<button class="layui-btn layui-btn-normal" style="height: 28px;line-height: 28px;background-color: #FD482C">
											审 核
										</button>
									</a>
								</c:if>
							    <c:if test="${obj.appStatus.valueId == 2 || obj.appStatus.valueId == 3 || obj.appStatus.valueName == '审核通过' || obj.appStatus.valueName == '审核未通过'}">
								    <button class="layui-btn layui-btn-normal" style="height: 28px;line-height: 28px;background-color: green">
									    已审核
								    </button>
							    </c:if>
							</td>
